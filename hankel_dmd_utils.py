...
def exact_dmd(gtot, thrshhld, window, ndsets):
    ...
    # Select important singular values based on the threshold
    sm = np.max(s)
    ...
    evals, evecs = np.linalg.eig(kmat)  # Compute eigenvalues and eigenvectors
    
    # Compute Koopman modes
    koopman_modes = ur @ evecs  # Direct calculation of Koopman modes
...
def hankel_dmd(raw_data, n_traj, traj_len, window, thrshhld):
    ...
    # Call exact_dmd to compute Koopman modes and matrix
